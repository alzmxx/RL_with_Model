...
class junction:
    ...
    def coordinate(self,threshold=30,C=-20,path_indicator):
        ...
        if len(self.outLanes)==2:
            for vehicle in self.temp_vehicle:
                if path_indicator>0.5:
                    traci.vehicle.setRoute(vehicle,[self.ID,self.outLanes[1],traci.vehicle.getID(vehicle)[-4:]])
                else:
                    traci.vehicle.setRoute(vehicle,[self.ID,self.outLanes[0],traci.vehicle.getID(vehicle)[-4:]])
...
class network:
    ...
    def __init__(self,path,ui,sumocfgPath,steptime=60):
        ...
        lowVals=np.array([0,0,0]*(len(self.junctions)))
        highVals=np.array([1,1,1]*(len(self.junctions)))
        self.action_space=spaces.Box(low=lowVals, high=highVals)
    ...
    def action(self,params):
        ...
        for i in range(len(self.junctions)): 
            threshold=params[3*i]*50
            C=params[3*i+1]*50*(-1)
            path_indicator=[3*i+2]
            self.junctions[i].restrictDrivingMode()
            toUpdate=self.junctions[i].detectArrival()
            if toUpdate:
                self.junctions[i].coordinate(threshold,C,path_indicator)
                for lane in toUpdate:
                    self.lanes[lane].updateFlow()
